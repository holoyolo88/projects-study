...
public class GamePanel extends JPanel implements Runnable, KeyListener {
	...
	public void run() {
		...
		//���� �����
		{
			...
			s = "continue pressing R to R E T R Y";
			length = (int) g.getFontMetrics().getStringBounds(s, g).getWidth();
			g.drawString(s, (WIDTH - length) / 2, HEIGHT / 2 + 60);
			
			s = "continue pressing ESC to E X I T";
			length = (int) g.getFontMetrics().getStringBounds(s, g).getWidth();
	...
	private void gameUpdate() {
		player.setPause(player.pause);
		player.setGpause(player.Gpause);
		...
		for(int i = 0; i < enemies.size(); i++) {
			if(enemies.get(i).isDead()) {
				...
				if(rand < 0.02) items.add(new Item(1, e.getx(), e.gety()));
				else if(rand < 0.04)items.add(new Item(5, e.getx(), e.gety())); //score-100 item 추가
				else if(rand < 0.07)items.add(new Item(6, e.getx(), e.gety())); //item 추가
				else if(rand < 0.1)items.add(new Item(4, e.getx(), e.gety())); //item 생성 확률 변경
				else if(rand < 0.2)items.add(new Item(3, e.getx(), e.gety())); //item 생성 확률 변경
				else if(rand < 0.3)items.add(new Item(2, e.getx(), e.gety())); //item 생성 확률 변경
				...
				e.explode();
				explosions.add(new Explosion(e.getx(), e.gety(), e.getr(), e.getr() + 25)); // 폭발 크기 변경
			}
		}
	...
	private void gameRender() {
		...
		for(int i = 0; i < player.getLifes(); i++){
			...
			g.drawString("Use rudder to move", 18, 65);
			g.drawString("Press space bar to attack", 18, 80);
			g.drawString("'S' to pause", 18, 95);
			g.drawString("'A' to resume", 18, 110);
	...
	public void keyPressed(KeyEvent e) {
		...
		if(keyCode == KeyEvent.VK_S) {
			...
			Player.setGpause(true);
		}
		if(keyCode == KeyEvent.VK_A) {
			Player.setGpause(false);
			for(int i = 0; i < enemies.size(); i++) {
