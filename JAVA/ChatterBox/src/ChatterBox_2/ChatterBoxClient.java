...
public class ChatterBoxClient extends Application {
	...
	void receiveResult() {
		while (!socket.isClosed()) {
			try {
				...
				Platform.runLater(() -> displayStatusText("Status | checked userData"));
				if (data.equals("true")) {
					Platform.runLater(() -> {
	...
	void receiveReceivedData() {
		
		File file = new File(id+"received");
		try {
			file.createNewFile();
	...
	public void start(Stage primaryStage) throws Exception {
		...
		btnConn.setOnAction(e -> {
			if (btnConn.getText().equals("Start")) {
				...
				stopClient();
			receivedTxtDisplay.setText("");
			sentTxtDisplay.setText("");
		});
